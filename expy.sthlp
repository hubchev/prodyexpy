...
This option has to be used if more than one EXPY is calculated and if the PRODY-variables do not start with "prody" or "PRODY".
To make this more clear, consider the following example:

{p 9 9 2}
Suppose several versions of capital-intensities have been calculated previously which are called "capint_v1", "capint_v2", etc., and are intended to be used for calculating "expy_v1", "expy_v2", etc. 
In order to enable the program {cmd:expy} to match and parse variable names it is essential to specify {cmd:nameprody(}{it:capint}{cmd:)}.
...
{col 5} Stephan Huber and Christoph Rust
